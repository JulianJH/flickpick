        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              autoCapitalize="none"
              spellCheck={false}
              className="w-full bg-lb-card border border-lb-border rounded-none px-4 py-3 text-lb-text placeholder:text-lb-muted focus:outline-none focus:border-lb-accent transition-colors text-base"
            />
            ...
            <div className="grid grid-cols-2 gap-2">
              {([8, 16] as MatchSize[]).map((s) => (
                <button
                  ...
                  type="button"
                  onClick={() => setSize(s)}
                  className={`py-3 rounded-none font-semibold text-sm transition-all border ${
                    size === s
          ...
          <motion.button
            whileTap={{ scale: 0.97 }}
            type="submit"
            className="w-full bg-lb-accent hover:bg-lb-accent-dim text-white font-bold py-4 rounded-none text-base transition-colors"
          >
            Let's Pick a Movie →
          </motion.button>
        </form>
