        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              autoCapitalize="none"
              spellCheck={false}
              className="w-full bg-lb-card border border-lb-border rounded px-4 py-3 text-lb-text placeholder:text-lb-muted focus:outline-none focus:border-lb-accent transition-colors text-base"
            />
            ...
            <div className="grid grid-cols-2 gap-2">
              {([8, 16] as MatchSize[]).map((s) => (
                <button
                  ...
                  type="button"
                  onClick={() => setSize(s)}
                  className={`py-3 rounded font-semibold text-sm transition-all border ${
                    size === s
